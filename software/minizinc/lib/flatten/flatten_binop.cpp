...
void collect_linexps(EnvI& env, typename LinearTraits<Lit>::Val in_c, Expression* exp,
  ...
  while (!stack.empty()) {
    ...
    if (e->type().isPar()) {
      ...
      if (call->id() == env.constants.ids.sum) {
        assert(call->argCount() == 1);
        ArrayLit* al = eval_array_lit(env, call->arg(0));
        for (int i = 0; i < al->size(); ++i) {
          stack.emplace_back((*al)[i], c);
        }
      } else if (call->id() == env.constants.ids.lin_exp) {
        assert(call->argCount() == 3);
        ArrayLit* ncoeffs = eval_array_lit(env, call->arg(0));
        ArrayLit* nvars = eval_array_lit(env, call->arg(1));
        for (int i = 0; i < ncoeffs->size(); ++i) {
          stack.emplace_back((*nvars)[i], c * LinearTraits<Lit>::eval(env, (*ncoeffs)[i]));
        }
        constval += LinearTraits<Lit>::eval(env, call->arg(2));
      } else {
        coeffs.push_back(c);
        vars.emplace_back(e);
      }
...
KeepAlive mklinexp(EnvI& env, typename LinearTraits<Lit>::Val c0,
                   typename LinearTraits<Lit>::Val c1, Expression* e0, Expression* e1, Id* obj) {
  typedef typename LinearTraits<Lit>::Val Val;
  GCLock lock;
  ...
  if (coeffs.empty()) {
  ...
  } else if (coeffs.size() == 1 && coeffs[0] == 1 && constval == 0 && !obj) {
    ka = vars[0];
  } else {
    ...
    std::vector<Expression*> args(3 + (obj != nullptr), nullptr);
    ...
    Call* c = new Call(e0->loc().introduce(),
                       obj ? env.constants.ids.obj_lin_eq : env.constants.ids.lin_exp, args);
...
template KeepAlive mklinexp<IntLit>(EnvI&, typename LinearTraits<IntLit>::Val,
                                    typename LinearTraits<IntLit>::Val, Expression*, Expression*,
                                    Id*);
template KeepAlive mklinexp<FloatLit>(EnvI&, typename LinearTraits<FloatLit>::Val,
                                      typename LinearTraits<FloatLit>::Val, Expression*,
                                      Expression*, Id*);
