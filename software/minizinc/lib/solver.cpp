...
SolverInitialiser::SolverInitialiser() {
  ...
  static SATSolverFactoryInitialiser _sat_init;
}
    ...
    if (sc.executable().empty()) {
      ...
      switch (sc.inputType()) {
        ...
        case SolverConfig::O_NL:
          ...
          break;
        case SolverConfig::O_DIMACS:
        case SolverConfig::O_WDIMACS:
          solverId = "org.minizinc.mzn-sat";
          break;
      }
    }
    bool found = false;
            ...
            if (sc.inputType() == SolverConfig::O_MZN) {
              ...
              switch (sc.inputType()) {
                ...
                case SolverConfig::O_NL:
                  ...
                  break;
                case SolverConfig::O_DIMACS:
                  additionalArgs.emplace_back("--sat-no-weights");
                  // fall through
                case SolverConfig::O_WDIMACS:
                  additionalArgs.emplace_back("--sat-cmd");
                  break;
                case SolverConfig::O_MZN:
              ...
              if (!fzn_mzn_flags.empty()) {
                switch (sc.inputType()) {
                  ...
                  case SolverConfig::O_DIMACS:
                  case SolverConfig::O_WDIMACS:
                    add_flags("--sat-flag", fzn_mzn_flags, additionalArgs);
                    break;
                  case SolverConfig::O_MZN:
                    assert(false);  // unreachable
                }
