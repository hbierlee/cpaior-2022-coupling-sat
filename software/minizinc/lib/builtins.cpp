...
bool b_annotate(EnvI& env, Call* call) {
  ...
  if (auto* ann_decl = follow_id_to_decl(ann)->dynamicCast<VarDecl>()) {
    ...
    if (var_it != env.varOccurrences.idx.end()) {
...
Expression* b_get_ann_arg(EnvI& env, Call* call) {
  assert(call->argCount() == 3);
  /* assert(b_has_ann(env,call)); */
  Expression* expr = call->arg(0);
  expr = follow_id_to_decl(expr);

  Expression* ann = call->arg(1);
  IntVal arg = eval_int(env, call->arg(2)).minus(1);
  auto* key = ann->cast<Call>();

  Call* ann_call = expr->ann().getCall(key->id());
  assert(arg >= 0 && arg < ann_call->argCount());
  return ann_call->arg(arg.toInt());
}
...
Expression* b_group(EnvI& env, Call* call) {
  assert(call->argCount() == 1);
  ArrayLit* al = eval_array_lit(env, call->arg(0));
  ...
  std::vector<IntVal> as(al->size());
  for (unsigned int i = al->size(); (i--) > 0;) {
    as[i] = eval_int(env, (*al)[i]);
  }
  ...
  // maps value -> the set of indices of that value
  std::map<IntVal, std::vector<IntVal>> map;

  // We keep a pointer cached
  std::vector<IntVal>* last_group = nullptr;
  IntVal last_value;
  for (size_t i = 0; i < as.size(); i++) {
    IntVal value = as[permutation[i]];

    // account for minimum index offset
    IntVal index = IntVal(al->min(0) + static_cast<long long int>(permutation[i]));

    if (i > 0 && value == last_value) {
      last_group->push_back(index);
    } else {
      // TODO we cache the last group, but in small tests this didn't seem to make a difference
      // (maybe compiler already caches last retrieved value from map)
      last_group = &map[value];
      last_group->push_back(index);
      last_value = value;
    }
  }
  ...
  // TODO use accumulate?
  std::vector<Expression*> grouped;

  // TODO reserve card. of dom_array of as
  // grouped.reserve()
  for (auto e : map) {
    auto* group = new SetLit(al->loc(), IntSetVal::a(e.second));
    grouped.push_back(group);
  }

  auto* al_grouped = new ArrayLit(al->loc(), grouped);

  al_grouped->type(Type::parsetint(1));
  return al_grouped;
}
...
void register_builtins(Env& e) {
  ...
  }
  {
    ...
    t[1] = Type::ann();
    t[2] = Type::parint();
    rb(env, m, ASTString("get_ann_arg"), t, b_get_ann_arg);
    t[0] = Type::varsetint();
    rb(env, m, ASTString("get_ann_arg"), t, b_get_ann_arg);
    Type setoftop;
    setoftop.bt(Type::BT_TOP);
  ...
  { rb(env, m, ASTString("showCheckerOutput"), {}, b_show_checker_output); }
  {
    ...
    t[3] = Type::varbool();
    t[4] = Type::varbool();
    rb(env, m, ASTString("build_bdd_le"), t, b_build_bdd_le);
  }
  {
    std::vector<Type> t(5);
    t[0] = Type::parint(1);
    ...
    t[3] = Type::varbool();
    t[4] = Type::varbool();
    rb(env, m, ASTString("build_bdd_eq"), t, b_build_bdd_eq);
  }
  {
    std::vector<Type> t(5);
    t[0] = Type::parint(1);
    ...
    t[3] = Type::varbool();
    t[4] = Type::varbool();
    rb(env, m, ASTString("build_bdd_ne"), t, b_build_bdd_ne);
  }
  {
    std::vector<Type> t(1);
    t[0] = Type::varbool(1);
    // t[1] = Type::parint(0); TODO for m par
    rb(env, m, ASTString("at_most1_pairwise"), t, b_at_most1_pairwise);
  }
  {
    std::vector<Type> t(1);
    t[0] = Type::varbool(1);
    // t[1] = Type::parint(0); TODO for m par
    rb(env, m, ASTString("at_most1_bimander"), t, b_at_most1_bimander);
  }
  {
    std::vector<Type> t(1);
    t[0] = Type::varbool(1);
    // t[1] = Type::parint(0); TODO for m par
    rb(env, m, ASTString("at_most1_mimander"), t, b_at_most1_mimander);
  }
  {
    std::vector<Type> t(4);
    t[0] = Type::varbool(1);  // xs
    t[1] = Type::parint();    // Comparator TODO make enum
    t[2] = Type::parint();    // cardinality
    t[3] = Type::parfloat();  // lambda
    rb(env, m, ASTString("build_sorting_network"), t, b_build_sorting_network);
  }
  {
    std::vector<Type> t(3);
    t[0] = Type::parint(0);
    t[1] = Type::parint(0);
    t[2] = Type::parbool(0);
    rb(env, m, ASTString("binary"), t, b_binary);
  }
  {
    std::vector<Type> t(1);
    t[0] = Type::parint(1);
    rb(env, m, ASTString("group"), t, b_group);
  }
