...
public:
  ...
  FlattenModelVisitor(EnvI& env0, VarDecl* _objective, ItemTimer::TimingMap* timingMap0)
      : env(env0), objective(_objective), timingMap(timingMap0) {}
  ...
  void vVarDeclI(VarDeclI* v) {
    ...
    if (v->e() == objective) {
      // This only works when all Ids in the objective expression (or rather their VarDecl) have
      // been flattened. Otherwise `eval_id` might fail in the `mklinexp` call. (This should be
      // fine since _objective is introduced last)
      linearize_objective(env, v->e());
    }
  ...
  void vDelayedSolveI(SolveI* si) {
    if (si == nullptr) {
      GCLock lock;
      env.flatAddItem(SolveI::sat(Location().introduce()));
    } else {
...
void flatten(Env& e, FlatteningOptions opt) {
  ...
  try {
    ...
    // Flatten main model
    SolveI* solveItem = env.model->solveItem();
    // In case of an optimisation objective, the objective expression has been placed in _objective
    assert(solveItem == nullptr || solveItem->st() == SolveI::SolveType::ST_SAT ||
           solveItem->e()->isa<Id>());

    FlattenModelVisitor _fv(env,
                            (solveItem != nullptr && solveItem->st() != SolveI::ST_SAT)
                                ? solveItem->e()->cast<Id>()->decl()
                                : nullptr,
                            timingMap);
    iter_items<FlattenModelVisitor>(_fv, e.model());
    _fv.vDelayedSolveI(solveItem);
...
void linearize_objective(EnvI& env, VarDecl* vd) {
  bool linearizeObj = false;
  {
    GCLock lock;
    Call* check_linearize_obj =
        new Call(Location(), "mzn_check_linearize_objective", std::vector<Expression*>());
    check_linearize_obj->type(Type::parbool());
    check_linearize_obj->decl(env.model->matchFn(env, check_linearize_obj, false));
    linearizeObj = eval_bool(env, check_linearize_obj);
  }
  if (!linearizeObj) {
    return;
  }
  ...
  KeepAlive lin_obj;
  if (vd->type().isfloat()) {
    lin_obj = mklinexp<FloatLit>(env, 1, 0, vd->e(), nullptr, vd->id());
  } else {
    lin_obj = mklinexp<IntLit>(env, 1, 0, vd->e(), nullptr, vd->id());
  }
  if (auto* c = lin_obj()->dynamicCast<Call>()) {
    GCLock lock;
    assert(c->id() == env.constants.ids.obj_lin_eq);
    auto* cons = new ConstraintI(vd->loc().introduce(), c);
    env.flatAddItem(cons);
    vd->e(nullptr);
  } else {
    // It must have been a constant
    vd->e(lin_obj());
  }
}
}  // namespace MiniZinc
